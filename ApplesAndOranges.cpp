...
void countApplesAndOranges(int s, int t, int a, int b, vector<int> apples, vector<int> oranges) {
    ...
    int applesInReach = 0;
    int orangesInReach = 0;
    
    for(const auto& p : apples){
        appleDistances.push_back(a+p);
    }
    for(const auto& p : oranges){
        orangeDistances.push_back(b+p);
    }
    for(const auto& p : appleDistances){
        if(p>=s && p<=t)
            applesInReach++;
    }
    for(const auto& p : orangeDistances){
        if(p>=s && p<=t)
            orangesInReach++;
    }
    cout<<applesInReach<<endl;
    cout<<orangesInReach<<endl;
}
